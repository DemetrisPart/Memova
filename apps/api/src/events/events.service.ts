...
export class EventsService {
  ...
  async updateEvent(
    ...
    const event = await this.prisma.event.update({
      ...
      data: {
        ...
        notifyExpiry3d: dto.notifyExpiry3d,
        notifyFirstGuestPhoto: dto.notifyFirstGuestPhoto,
        notifyStorageNearFull: dto.notifyStorageNearFull,
      },
      include: { coverImage: true },
    });
  ...
  private async serializePublicEvent(event: EventWithCover) {
    ...
    return {
      ...
      galleryVisibleDays: event.galleryVisibleDays,
      storageUsedBytes: event.storageUsedBytes.toString(),
      storageLimitBytes: event.storageLimitBytes.toString(),
      storageUsedPercent,
      ...coverUrls,
  ...
  async serializeEvent(
    ...
    return {
      ...
      showGuestNamesPublicly: event.showGuestNamesPublicly,
      galleryVisibleDays: event.galleryVisibleDays,
      notifyExpiry3d: event.notifyExpiry3d,
      notifyWeddingCountdown: event.notifyWeddingCountdown,
      notifyFirstGuestPhoto: event.notifyFirstGuestPhoto,
      notifyStorageNearFull: event.notifyStorageNearFull,
      storageUsedBytes: event.storageUsedBytes.toString(),
      storageLimitBytes: event.storageLimitBytes.toString(),
